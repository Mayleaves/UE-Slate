...
/** 自定义作业属性在详细信息面板中的显示方式 */
// 显示 Job Name、Sequence、Map
class FMyJobDetailsCustomization final : public IDetailCustomization
{
public:
	static TSharedRef<IDetailCustomization> MakeInstance()
	{
		return MakeShared<FMyJobDetailsCustomization>();
	}

	virtual void FMyJobDetailsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder) override
	{
		// 需要显示的属性
		const TSharedRef<IPropertyHandle> JobNameHandle = DetailBuilder.GetProperty(
			TEXT("JobName"), UMoviePipelineExecutorJob::StaticClass());
		const TSharedRef<IPropertyHandle> SequenceHandle = DetailBuilder.GetProperty(
			TEXT("Sequence"), UMoviePipelineExecutorJob::StaticClass());
		const TSharedRef<IPropertyHandle> MapHandle = DetailBuilder.GetProperty(
			TEXT("Map"), UMoviePipelineExecutorJob::StaticClass());

		// 将属性添加到自定义分类，这里设置为 Job Info
		IDetailCategoryBuilder& CustomCategory = DetailBuilder.EditCategory("Job Info");
		CustomCategory.AddProperty(JobNameHandle);
		CustomCategory.AddProperty(SequenceHandle);
		CustomCategory.AddProperty(MapHandle);

		// 隐藏不需要的属性
		DetailBuilder.HideCategory("Movie Render Pipeline");
		DetailBuilder.HideCategory("Variable Assignments");
		// ...继续添加你想隐藏的属性
	}
};

// // 完整版
// class FMyJobDetailsCustomization : public IDetailCustomization
// {
// public:
// 	static TSharedRef<IDetailCustomization> MakeInstance()
// 	{
...
// 	}
//
// 	virtual ~FMyJobDetailsCustomization() override
// 	{
// 	}
//
...
// 		// a save occurs, but this would be very difficult to get right when multiple subgraphes are involved.
// 		UPackage::PackageSavedWithContextEvent.AddSP(this, &FMyJobDetailsCustomization::RefreshLayout);
//
...
// 			}
// 			else if (UMoviePipelineExecutorShot* SelectedShotTemp = Cast<UMoviePipelineExecutorShot>(
// 				SelectedObject.Get()))
// 			{
...
// 			InDetailBuilder.GetProperty(
// 				TEXT("PrimaryGraphVariableAssignments"), UMoviePipelineExecutorShot::StaticClass());
...
// 			SelectedShot->OnShotGraphPresetChanged.AddSP(this, &FMyJobDetailsCustomization::RefreshLayout);
//
...
// 				SelectedJob->OnJobGraphPresetChanged.AddSP(this, &FMyJobDetailsCustomization::RefreshLayout);
// 			}
// 		}
// 		else
// 		{
...
// 			"PrimaryGraphVariablesShotOverrides", LOCTEXT("PrimaryGraphVariablesShotOverridesCategory",
// 			                                              "Primary Graph Variables (shot overrides)"));
...
// 		IDetailCategoryBuilder& ConsoleVariableOverridesCategory = InDetailBuilder.EditCategory(
...
//
// 	/** The shot that's selected in the UI (maybe null if only a primary is selected). */
